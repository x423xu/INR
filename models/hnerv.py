...
class HNeRV(nn.Module):
    def __init__(self, args):
        ...
        # get arguments for normlayer
        if args.norm == 'ln':
            if args.resize_list == "-1":
                h,w = [int(i) for i in args.crop_list.split('_')[:2]]
                for red in args.enc_strds:
                    h,w = h//red, w//red
                dec_embed_size = [(h,w)]
                for i in args.dec_strds:
                    h,w = h*i, w*i
    ...
    def forward(self, input, input_embed=None, encode_only=False, epoch=1):
        ...
        # bunny_flatten = bunny_flatten[:,:,torch.randperm(bunny_flatten.size(2))]
        # img_embed  = bunny_flatten.reshape(img_embed.shape)   
...
class NeRVBlock(nn.Module):
    def __init__(self, **kargs):
        ...
        if kargs['norm'] == 'ln':
            self.norm = NormLayer(kargs['norm'], kargs['dec_embed_size'])
        else:
...
def NormLayer(norm_type, ch_width):    
    if norm_type == 'none':
        ...
        norm_layer = nn.LayerNorm(ch_width)
    else:
        raise NotImplementedError
    ...
    def __init__(self, **kargs):
        ...
        if  kargs['conv_type']  == 'pshuffel':
            self.upconv = nn.Sequential(
                ...
                # nn.Conv2d(new_ngf, new_ngf, ks, 1, ceil((ks - 1) // 2), bias=kargs['bias']),
                # nn.GELU(),
                # nn.Conv2d(new_ngf, new_ngf, ks, 1, ceil((ks - 1) // 2), bias=kargs['bias']),
            )
        elif  kargs['conv_type']  == 'conv':
